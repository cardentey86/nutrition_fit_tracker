...
class PechoChartWidget extends StatefulWidget {
  final VoidCallback onClose;
  const PechoChartWidget({super.key, required this.onClose});

  @override
  State<PechoChartWidget> createState() => _MyWidgetState();
}

class _MyWidgetState extends State<PechoChartWidget> {
    ...
    if (result.isNotEmpty) {
      ...
      for (var element in result) {
        progressValues.add(ProgresoValores(
            date: element.fecha.toString(), value: element.pecho));
      }

      model = ProgressModel(
          item: 'progress.chestChart'.tr(), progressValues: progressValues);
    }
    if (!mounted) return;
    setState(() {
      _isLoading = false;
        ...
        child: Stack(children: [
          SfCartesianChart(
            ...
            onTooltipRender: (TooltipArgs args) {
              ...
              args.text = '$fecha';
              args.header =
                  '${'progress.chestChart'.tr()}: ${peso.toStringAsFixed(1)}';
            },
            title: ChartTitle(
              text: model!.item,
              textStyle:
            ...
            primaryYAxis: NumericAxis(
              title: AxisTitle(text: 'progress.chestChart'.tr()),
              isVisible: false,
              labelStyle: const TextStyle(fontSize: 12, color: Colors.grey),
              axisLine: const AxisLine(color: Colors.grey),
