...
class PgcChartWidget extends StatefulWidget {
  final VoidCallback onClose;
  const PgcChartWidget({super.key, required this.onClose});

  @override
  State<PgcChartWidget> createState() => _MyWidgetState();
}

class _MyWidgetState extends State<PgcChartWidget> {
    ...
    if (result.isNotEmpty) {
      ...
      for (var element in result) {
        progressValues.add(ProgresoValores(
            ...
            value: await indices.porcientoGrasaJacksonPollock(element)));
      }

      model = ProgressModel(
          item: 'progress.pgcChart'.tr(), progressValues: progressValues);
    }
    if (!mounted) return;
    setState(() {
      _isLoading = false;
        ...
        child: Stack(children: [
          SfCartesianChart(
            ...
            onTooltipRender: (TooltipArgs args) {
              ...
              args.text = '$fecha';
              args.header = 'PGC: ${peso.toStringAsFixed(1)}';
            },
            title: ChartTitle(
              text: model!.item,
              textStyle:
            ...
            primaryYAxis: NumericAxis(
              title: AxisTitle(text: 'progress.pgcChart'.tr()),
              isVisible: false,
              labelStyle: const TextStyle(fontSize: 12, color: Colors.grey),
              axisLine: const AxisLine(color: Colors.grey),
