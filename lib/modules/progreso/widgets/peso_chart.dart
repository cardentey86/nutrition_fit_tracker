...
class _MyWidgetState extends State<PesoChartWidget> {
  ...
  Future<void> loadData() async {
    ...
    if (result.isNotEmpty) {
      ...
      model = ProgressModel(
          item: 'progress.weightChart'.tr(), progressValues: progressValues);
    }
    if (!mounted) return;
    setState(() {
      _isLoading = false;
        ...
        child: Stack(children: [
          SfCartesianChart(
            ...
            onTooltipRender: (TooltipArgs args) {
              ...
              args.text = '$fecha';
              args.header =
                  '${'progress.weightChart'.tr()}: ${peso.toStringAsFixed(1)} kg';
            },
            title: ChartTitle(
              text: model!.item,
              textStyle:
            ...
            primaryYAxis: NumericAxis(
              title: AxisTitle(text: 'progress.weightChart'.tr()),
              isVisible: false,
              labelStyle: const TextStyle(fontSize: 12, color: Colors.grey),
              axisLine: const AxisLine(color: Colors.grey),
